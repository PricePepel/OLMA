...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Test Find Teachers - Current user:', user.id)
    ...
    console.log('Test Find Teachers - User learning skills:', userLearningSkills)

    if (!userLearningSkills || userLearningSkills.length === 0) {
      return NextResponse.json({ 
        message: 'No skills to learn found',
        userLearningSkills: [],
        matches: []
      })
    }
    ...
    console.log('Test Find Teachers - Skill IDs to find teachers for:', skillIds)

    if (skillIds.length === 0) {
      return NextResponse.json({ 
        message: 'No valid skill IDs found',
        skillIds: [],
        matches: []
      })
    }
    ...
    console.log('Test Find Teachers - Potential teachers found:', potentialTeachers)
    ...
    console.log(`Test Find Teachers - Final matches: ${matches.length} teachers found`)
    ...
    return NextResponse.json({ 
      ...
      matches,
      summary: {
        skillsUserWantsToLearn: userLearningSkills.length,
        skillIdsFound: skillIds.length,
        potentialTeachersFound: potentialTeachers?.length || 0,
        finalMatches: matches.length
      }
    })
  } catch (error) {
    console.error('Test Find Teachers error:', error)
